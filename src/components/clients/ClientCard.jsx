...
export default function ClientCard({ client, colorIdx, onEdit, onDelete }) {
  ...
  return (
    <div className={`group ${glass} rounded-2xl p-5 hover:border-black/30 dark:hover:border-white/20 transition-all duration-300 relative overflow-hidden`}>
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {attrs.company && <p className="text-black/60 dark:text-white/40 text-xs">{attrs.company}</p>}
          </div>
        </div>
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => onEdit(client)}
            className="w-7 h-7 rounded-lg bg-white/5 hover:bg-violet-500/20 text-black/40 dark:text-white/40 hover:text-violet-400 flex items-center justify-center transition-all">
            <Edit2 size={12} />
          </button>
          <button onClick={() => onDelete(client.documentId)}
            className="w-7 h-7 rounded-lg bg-white/5 hover:bg-red-500/20 text-black/40 dark:text-white/40 hover:text-red-400 flex items-center justify-center transition-all">
            <Trash2 size={12} />
          </button>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {attrs.email && (
          <div className="flex items-center gap-2 text-xs text-black/70 dark:text-white/45">
      ...
      {attrs.notes && (
        <p className="mt-3 pt-3 border-t border-black/20 dark:border-white/5 text-black/50 dark:text-white/30 text-xs leading-relaxed line-clamp-2">
          {attrs.notes}
        </p>
      )}
    </div>
